...
const Card = ({ children, className = "", glow = false }) => (
  <div
    className={`relative rounded-xl border border-slate-800 bg-slate-900/60 bac
kdrop-blur-sm ${
...
const SectionLabel = ({ children, num }) => (
  <div className="mb-6 flex items-center gap-3">
    ...
    <span className="font-mono text-[11px] tracking-[0.25em] uppercase text-sla
te-400">
      {children}
    </span>
  </div>
...
const StatRow = ({ label, value, accent = "" }) => (
  <div className="flex items-baseline justify-between border-b border-slate-800
/60 py-2.5 last:border-b-0">
    <span className="text-[11px] uppercase tracking-wider text-slate-500">
      {label}
    </span>
    <span className={`font-mono text-sm tabular-nums ${accent || "text-slate-20
0"}`}>
      {value}
    </span>
  </div>
  ...
  return (
    ...
    >
      <style>{`
        ...
        input[type=range].sss-slider::-moz-range-track {
          height: 4px; background: #1e293b; border-radius: 999px;
        }
        input[type=range].sss-slider::-moz-range-progress {
          height: 4px; background: #0ea5e9; border-radius: 999px;
        }
        input[type=range].sss-slider::-webkit-slider-thumb {
          ...
          margin-top: -9px;
          box-shadow: 0 0 0 6px rgba(14,165,233,0.15), 0 0 30px rgba(14,165,233,0.5);
          cursor: grab;
          transition: transform .15s ease;
        }
        input[type=range].sss-slider::-webkit-slider-thumb:active { transform: scale(1.1); cursor: grabbing; }
        input[type=range].sss-slider::-moz-range-thumb {
          width: 22px; height: 22px; border-radius: 999px;
          background: #f8fafc; border: 3px solid #0ea5e9; cursor: grab;
        }
      `}</style>
      ...
      <header className="relative border-b border-slate-800/80 grid-bg">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-sky-500 to-sky-700 flex items-center justify-center font-bold text-white text-lg tracking-tighter shadow-[0_0_30px_-6px_rgba(14,165,233,0.6)]">
                S³
              </div>
            </div>
            <div className="leading-tight">
              <div className="font-bold tracking-tight text-slate-50 text-[15px]">
                SSS MARKETING
              </div>
              <div className="mono text-[10px] tracking-[0.2em] text-slate-500 uppercase">
                Capital Partners
              </div>
            </div>
          </div>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <div className="text-[11px] uppercase tracking-wider text-slate-500">
              Confidential · Series Seed Memorandum
            </div>
        ...
        <div className="relative mx-auto max-w-7xl px-6 pb-12 pt-8">
          <div className="flex items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-sky-400/90 mb-4">
            <Briefcase className="w-3.5 h-3.5" />
            Partner Equity Dashboard
          </div>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-slate-50 leading-[1.05] max-w-4xl">
            A <span className="text-sky-400">$30,000</span> position in the{" "}
            <span className="italic text-slate-300">utility layer</span> of
            Canadian dental care.
          </h1>
          <p className="mt-5 max-w-2xl text-slate-400 leading-relaxed">
            Partner subscribes for a{" "}
            <span className="text-slate-200 font-semibold">
              15% perpetual net-profit equity
            </span>
            {" "}in SSS Marketing's clinic-services platform. The 24-month model
The file continues beyond 400 lines. But we will patch by replacing entire inline script starting at line where <script type=